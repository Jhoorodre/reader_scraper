...
import { MangaLivreProvider } from '../providers/scans/mangalivre';
...
async function downloadManga() {
    ...
    const provider = new MangaLivreProvider();
    const chapterLogger = new ChapterLogger();
    const reportFile = 'download_report.txt';

    try {
        console.log('\n' + '='.repeat(80));
        console.log('🇧🇷 MANGALIVRE.TV DOWNLOADER');
        console.log('='.repeat(80));

        const mangaUrl = await promptUser('Digite a URL da obra do MangaLivre: ') as string;

        // Validar URL
        if (!mangaUrl.includes('mangalivre.tv')) {
            throw new Error('URL deve ser do MangaLivre.tv');
        }
        ...
        fs.writeFileSync(reportFile, `Relatório de Download - MangaLivre.tv\nManga: ${manga.name}\nURL: ${manga.id}\n\n`);
        ...
        console.log('\n📋 Opções de Download:');
        console.log('1. Baixar todos os capítulos');
        console.log('2. Baixar por faixa (ex: 1-10)');
        console.log('3. Baixar capítulo específico');
        
        const downloadOption = await promptUser('Escolha uma opção (1-3): ') as string;
        ...
        switch (downloadOption) {
            ...
            case '2':
                const range = await promptUser('Digite a faixa (ex: 1-10): ') as string;
                ...
                chaptersToDownload = chapters.slice(start, end + 1);
                break;
        ...
        const mangaDir = getMangaPathForSite('MangaLivre', manga.name);
